...
#        ifdef BABL_MOVE
    ...
    BABLM(BABL_DEL_LINE, SS_TAP(X_HOME) SS_TAP(X_HOME) IMSFT(X_END) SS_TAP(X_DELETE) SS_TAP(X_BSPACE) SS_TAP(X_RIGHT));
...
#        ifdef BABL_BROWSER
    ...
    BABLM(BABL_BROWSER_CLOSE_TAB, IMCTL(X_F4));
    BABLM(BABL_BROWSER_REOPEN_LAST_TAB, OMSFT(IMCTL(X_T)));
    BABLM(BABL_BROWSER_NEXT_TAB, IMCTL(X_PGDOWN));
    BABLM(BABL_BROWSER_PREV_TAB, IMCTL(X_PGUP));
...
#        ifdef BABL_IDE
    ...
    // BABL_IDE_RECENT_LOC;
    BABLM(BABL_IDE_FILE_FIND, OMSFT(IMCTL(X_F)));
    // BABL_IDE_REL_FILE;
    // BABL_IDE_REL_SYM;
    BABLM(BABL_IDE_EXTEND_SEL, SS_LCTL("w"));
    BABLM(BABL_IDE_COMPLETION, OMSFT(IMCTL(X_SPACE)));
    // BABL_IDE_FIXES;
    BABLM(BABL_IDE_QUICK_DOC, SS_LCTL("q"));
    // BABL_IDE_GO_LAST_ED;
