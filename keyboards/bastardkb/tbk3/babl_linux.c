...
#        ifdef BABL_OSKEYS
    ...
    /* BABLM( BABL_REPLACE , (SS_LCTL("r"))	); // KDE */
    BABLM(BABL_REPLACE, SS_LCTL("h"));  // Gnome*/
    BABLM(BABL_RUNAPP, IMALT(X_F2));         // Gnome
    BABLM(BABL_APP_NEXT, IMALT(X_TAB));
    BABLM(BABL_APP_LAST, OMSFT(IMALT(X_TAB)));
    BABLM(BABL_WIN_NEXT, OMCTL(IMALT(X_PGUP)));  // Gnome, sometimes
    BABLM(BABL_WIN_PREV, OMCTL(IMALT(X_PGDOWN)));
...
#        ifdef BABL_BROWSER
    ...
    BABLM(BABL_BR_CL_TAB, IMCTL(X_F4));
...
#        endif
#        ifdef BABL_IDE
    BABLM(BABL_I_WIN_FTREE, IMALT(X_1));
    BABLM(BABL_I_WIN_SEA, IMALT(X_3));
    BABLM(BABL_I_WIN_RUN, IMALT(X_4));
    BABLM(BABL_I_WIN_STR, IMALT(X_7));
    BABLM(BABL_I_WIN_GHIS, IMALT(X_9));
    BABLM(BABL_I_WIN_GCOM, IMALT(X_0));
    BABLM(BABL_I_REC_FILE, IMCTL(X_E));
    BABLM(BABL_I_REC_LOC, OMSFT(IMCTL(X_E)));
    BABLM(BABL_I_FINDA, SS_LCTL("l"));
    BABLM(BABL_I_FINF, OMSFT(IMCTL(X_F)));
    BABLM(BABL_I_FACTION, OMSFT(IMCTL(X_A)));
    BABLM(BABL_I_REL_FILE, SS_LCTL("t"));             // RubyMine goto-view / controller
    BABLM(BABL_I_REL_SYM, OMALT(IMCTL(X_HOME)));
    BABLM(BABL_I_EX_SEL, SS_LCTL("w"));
    BABLM(BABL_I_COMPLETE, OMSFT(IMCTL(X_SPACE)));
    BABLM(BABL_I_FIXES, IMALT(X_ENTER));
    BABLM(BABL_I_QDOC, SS_LCTL("q"));
    BABLM(BABL_I_GO_L_ED, OMSFT(IMCTL(X_BSPACE)));
    BABLM(BABL_I_GO_DEF, SS_LCTL("b"));
    BABLM(BABL_I_GO_BACK, OMALT(IMCTL(X_LEFT)));
    BABLM(BABL_I_GO_FWD, OMALT(IMCTL(X_RIGHT)));
    BABLM(BABL_I_GO_LINE, OMSFT(OMALT(IMCTL(X_G))));
    BABLM(BABL_I_GO_FILE, OMSFT(IMCTL(X_N)));
    BABLM(BABL_I_NEXT_FN, IMALT(X_DOWN));
    BABLM(BABL_I_PREV_FN, IMALT(X_UP));
    BABLM(BABL_I_USAGES, IMALT(X_F7));
    BABLM(BABL_I_RENAME, IMSFT(X_F6));
    BABLM(BABL_I_REFACTOR, OMSFT(OMALT(IMCTL(X_T))));
    BABLM(BABL_I_COMMENT, IMCTL(X_SLASH));
    BABLM(BABL_I_REFORMAT, OMALT(IMCTL(X_L)));
    BABLM(BABL_I_COL_MODE, OMALT(IMSFT(X_INSERT)));
    BABLM(BABL_I_NEW_LINE, IMSFT(X_ENTER));
    BABLM(BABL_I_DUP_LINE, IMCTL(X_D));
    BABLM(BABL_I_TGL_CASE, OMSFT(IMCTL(X_U)));
    BABLM(BABL_I_BUILD, IMCTL(X_F9));
    BABLM(BABL_I_RUNL, IMSFT(X_F10));
    BABLM(BABL_I_RUNS, OMSFT(IMALT(X_F10)));
    BABLM(BABL_I_RUNA,  OMSFT(OMALT(IMCTL(X_A))));
#        endif
